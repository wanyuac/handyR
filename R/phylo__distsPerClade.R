...
#' @param t An input bifurcating phylogenetic tree
...
distsPerClade <- function(t, dec = 8) {
    require(ape)
    require(phytools)
    if (! is.rooted(t)) {
        print("The input tree is unrooted and hence is rerooted using the midpoint.")
        t <- ladderize(phy = midpoint.root(t), right = FALSE)
    }
    taxa_num <- length(t$tip.label)
    nodes_in <- (taxa_num + 1) : (taxa_num + t$Nnode)  # IDs of internal nodes
    D <- do.call(rbind.data.frame, lapply(nodes_in, .summariseDistsPerClade, tr = t))
